...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchOrders(page = 1) {
    try {
      const response = await fetch(`/api/order/history/?page=${page}&page_size=${pageSize}`, {
        credentials: "include",
      });

      if (response.status === 401) {
        noOrdersMessage.innerText = "Silakan login untuk melihat riwayat pesanan.";
        noOrdersMessage.style.display = "block";
        return;
      }
      ...
      console.error(error);
      showToast("Terjadi kesalahan saat memuat pesanan");
    }
  }
